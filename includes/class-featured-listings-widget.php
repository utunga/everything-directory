 ...
 */
class EverythingDirectory_Featured_Listings_Widget extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'featured-listings', 'description' => __( 'Display grid-style featured listings', 'everything-directory' ) );
		$control_ops = array( 'width' => 300, 'height' => 350 );
		parent::__construct( 'featured-listings', __( 'EverythingDirectory - Featured Listings', 'everything-directory' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			if ( have_posts() ) : while ( have_posts() ) : the_post();
				...
				$loop .= sprintf( '<a href="%s" class="more-link">%s</a>', get_permalink(), __( 'View Listing', 'everything-directory' ) );

				$toggle = $toggle == 'left' ? 'right' : 'left';

				/** wrap in post class div, and output **/
				printf( '<div class="%s"><div class="widget-wrap"><div class="listing-wrap">%s</div></div></div>', join( ' ', get_post_class( $toggle ) ), apply_filters( 'everythingdir_featured_listings_widget_loop', $loop ) );
	...
	function form( $instance ) {
		...
		printf( '<p>%s <input type="text" name="%s" value="%s" size="3" /></p>', __( 'How many results should be returned?', 'everything-directory' ), $this->get_field_name('posts_per_page'), esc_attr( $instance['posts_per_page'] ) );
